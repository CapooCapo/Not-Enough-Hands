...
- `tests/hunter_slash_smoke.gd` **passes now**. It was written against a Huntsman retune that never landed, and its `_test_backpedal_cannot_escape_a_committed_grab` sub-test was arithmetically impossible for as long as the wind-up braked: 0.5 s at `Player.walk_speed` carried a backpedalling player clear of `seize_kill_radius` from any commit range. The balance decision that closed it was making the wind-up a **lunge** (`seize_lunge_speed` / `seize_lunge_turn_speed` in `ghosts/hunter_ghost.gd`) rather than touching the kill radius or the player. Its last sub-test pins the other side of that: a full sprint straight backwards still escapes, by centimetres.

- **One test already fails on a clean checkout**, so don't spend time thinking you broke it:
