...
func _run() -> void:
	...
	if not is_equal_approx(clock.real_seconds_per_game_minute, 2.5):
		_fail("Night clock should default to 2.5 real seconds per game minute.")
		return
	if clock.get_formatted_time() != "11:55 PM":
		_fail("Night clock did not start at 11:55 PM.")
		return

	clock.advance_real_seconds(2.499)
	if clock.get_formatted_time() != "11:55 PM":
		_fail("Clock advanced before a full 2.5 real seconds elapsed.")
		return
	clock.advance_real_seconds(0.001)
	if clock.get_formatted_time() != "11:56 PM":
		_fail("2.5 real seconds did not advance exactly one game minute.")
		return

	clock.advance_real_seconds(2.5 * 5.0)
	if clock.get_formatted_time() != "12:01 AM":
		_fail("Clock did not roll from 11:59 PM to 12:00 AM correctly.")
		return
	clock.advance_real_seconds(2.5)
	if clock.get_formatted_time() != "12:02 AM":
		_fail("12:01 AM to 12:02 AM did not take exactly 2.5 seconds.")
		return

	var victory_count := [0]
	clock.victory_reached.connect(func() -> void: victory_count[0] += 1)
	clock.advance_real_seconds(float(clock.get_minutes_remaining()) * 2.5)
	...
	print("Night clock smoke test passed: 2.5-second tick, midnight rollover, and 6:00 AM victory.")
	quit()
